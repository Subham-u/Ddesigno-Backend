...
export const getAdderesss = asyncHandler(async (req, res) => {
  const user = req.user;
  if (!user) {
    throw new ApiError(400, "User Not Found");
  }
  const getAdderesss = await Address.find({
    user: user._id,
  });
  if (!getAdderesss) {
    throw new ApiError(200, "some Issue Occured");
  }
  return res
    .status(200)
    .json(new ApiResponse(200, getAdderesss, "fetched addresss"));
});

export const addItemToCart = asyncHandler(async (req, res) => {
  const user = req.user;
  const { product, selectedAttributes, quantity, originalPrice, offerdPrice } =
    req.body;
  console.log();
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, newCartItem, "Cart fetched Successfully"));
});
...
export const deleteItemToCart = asyncHandler(async (req, res) => {
  const user = req.user;
...
export const getCartItems = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, cartItems, "Cart Item fetched successfully"));
});
...
export const getwishlistItems = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, products, "Wishlist Item fetched successfully"));
});
